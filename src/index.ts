...
function go(ev: Event) {
  ...
  for (const op_ of Object.keys(ops)) {
...
let op: (params: string[], ops: any) => void;
op = () => {};
let params = '';
if (sumParams) {
  op = sum;
    ...
    view: () =>
      m('.form', [
        ...
        m(
          'p',
          { onclick: go },
          Object.entries(ops).map(([op, opName]) =>
            m('button', { 'data-op': op }, opName),
          ),
        ),
        m('p', [
          m('br'),
          m(
            'a',
            { href: 'https://github.com/JosePedroDias/basic-algebra' },
